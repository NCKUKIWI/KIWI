...
router.get('/', function (req, res) {
    ...
    db.GetColumn('course_new', columns, {
        'column': 'id',
        'order': 'DESC'
    }, function (courses) {
        all_courses = courses;
...
router.post('/inputaddcourse/:courseid', function (req, res) {
    ...
    /* 透過輸入的選課序號 查找課程 */
    db.FindbyColumn('course_new', column, {
        '選課序號': courseserial
    }, function (course) {
        ...
        redis.get(courseCacheKey(id), function (err, reply) {
            if (reply) {
                ...
                db.query_post2(id, function (courseInfo, comment) {
                    ...
                    db.FindbyColumn('course_rate', ["*"], {
                        course_name: courseInfo.課程名稱,
                        teacher: courseInfo.老師
                    }, function (rates) {
                        var sweet = 0;
                        var hard = 0;
                        var recommand = 0;
...
function check_Login(req, res, all_courses, custom_courses) {
    if (req.session.user) {
        ...
        /* 有登入 抓取用戶的選課清單 */
        db.FindbyColumn('cart', ['course_id'], {
            'user_id': userid
        }, function (carts) {
